...
- **`triggers` en la definición del pipeline** — necesario para que se dispare solo en cada push. Ojo con esto: para fuentes GitHub/Bitbucket/GitLab vía `CodeStarSourceConnection`, la detección de cambios la maneja la propia Connection (webhook de la GitHub App), **no EventBridge** (eso es solo para CodeCommit/S3). Al crear el pipeline vía CLI en vez de por consola, el bloque `triggers` no se agrega solo — hay que declararlo explícitamente (`providerType: CodeStarSourceConnection`, filtro de `push` sobre `branches: main`), si no el pipeline solo corre con `start-pipeline-execution` manual.
