#!/bin/bash
# Deploy completo de gastos-query Lambda + API Gateway
# Requiere AWS CLI configurado con permisos suficientes

set -e

FUNCTION_NAME="gastos-query"
...
echo ""
echo "============================================"
echo " PASO 4: Crear ruta GET /expenses"
echo "============================================"
...
EXISTING_ROUTE=$(aws apigatewayv2 get-routes \
    --api-id "$API_ID" \
    --region "$REGION" \
    --query "Items[?RouteKey=='GET $ROUTE_PATH'].RouteId" \
    --output text)
...
if [ -n "$EXISTING_ROUTE" ]; then
    ...
    echo "Creando ruta GET $ROUTE_PATH..."
...
aws lambda add-permission \
    --function-name "$FUNCTION_NAME" \
    --statement-id "apigateway-get-expenses" \
